    ...
    for batch_idx, data in enumerate(testloader):
        ...
        for t in range(1, T):
            with torch.no_grad():
                ...
                t1 = time.time()
                # memorize
                key, val, _ = model(frames=frames[t-1:t], mask=tmp_mask, num_objects=num_objects)
            ...
            pred.append(out)
# comment out online optimization from original STM-cycle code
#            if t % opt.save_freq == 0:
#                # internal loop
#                for m in range(opt.loop):
#                    tmp_mask = out.detach().clone()
#                    tmp_mask.requires_grad = True
#                    key, val, _ = model(frames=frames[t:t+1], mask=tmp_mask, num_objects=num_objects)
#                    flogits, _, _, _, _ = model(frames=[None, frames[ref_index:ref_index+1]], prev_mask=None, keys=key, values=val,
#                     num_objects=num_objects, max_obj=max_obj)
#
