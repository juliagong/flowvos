...
def main():
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        if (epoch + 1) % opt.freeze_seg_every == 0:
            req_grad = ((epoch + 1) // opt.freeze_seg_every) % 2 == 1  # if odd, unfreeze
            s = net.state_dict()
            for k in s:  # only manipulate non-flow-related weights
                if s[k].dtype != torch.int64 and not 'flow_mixer' in k and not 'FlowRegressor' in k:
                    s[k].requires_grad = req_grad
        ...
        if (epoch + 1) % opt.epoch_per_test == 0:
            ...
            # calculate val dice, save checkpoints and results if best epoch
            val_mean, _ = eval_results(os.path.join(ROOT, opt.output_dir, opt.valset), os.path.join(ROOT, opt.valset))
            if val_mean > best_mean:
                best_dir = os.path.join(opt.checkpoint, 'best')
                ...
                save_checkpoint({
                    ...
                    'max_skip': skips,
                }, epoch + 1, checkpoint=best_dir, filename=opt.mode)

                best_output_dir = os.path.join(ROOT, opt.output_dir, opt.valset, 'best')
                if not os.path.exists(best_output_dir):
                    os.mkdir(best_output_dir)
                file_list = os.listdir(os.path.join(ROOT, opt.output_dir, opt.valset))
                for f in file_list:
                    if f != 'best':
                        if os.path.isdir(os.path.join(ROOT, opt.output_dir, opt.valset, f)):
                            if os.path.exists(os.path.join(best_output_dir, f)):
                                shutil.rmtree(os.path.join(best_output_dir, f))
                            shutil.copytree(os.path.join(ROOT, opt.output_dir, opt.valset, f),
                                            os.path.join(best_output_dir, f))
                        elif os.path.isfile(os.path.join(ROOT, opt.output_dir, opt.valset, f)):
                            shutil.move(os.path.join(ROOT, opt.output_dir, opt.valset, f),
                                        os.path.join(best_output_dir, f))

                best_mean = val_mean
                log.info('Best val mean J&F achieved: {}, epoch {}'.format(best_mean, epoch + 1))
        ...
        for idx in range(N):
            ...
            for t in range(1, T):
                ...
                total_loss = total_loss + criterion(out, gt, num_objects, ref=mask[0:1, :num_objects+1]) + \
                                          fcriterion(warpm, gt, num_objects, ref=mask[0:1, :num_objects+1]) + \
                                          mse_sim_loss(warpf, frame[t:t+1], (mask[t:t+1][:,1:].sum(1, keepdims=True) > 0))

            # cycle-consistancy
            key, val, r4 = model(frames=frame[T-1:T], mask=out, num_objects=num_objects)
            keys.append(key)
            vals.append(val)
            ...
            for t in range(T-1, 0, -1): 
                ...
                if convert_one_hot(cm, max_obj).max() == num_objects:
                    ...
                    logits, ps, warpm, warpf, flow = model(frames=[frame[t:t+1], frame[0:1]], prev_mask=mask[t:t+1] if tf else pred[t], keys=tmp_key, values=tmp_val,
                        num_objects=num_objects, max_obj=max_obj)
                    first_out = torch.softmax(logits, dim=1)
                    cycle_loss += criterion(first_out, mask[0:1], num_objects, ref=mask[t:t+1, :num_objects+1]) + \
                                          fcriterion(warpm, mask[0:1], num_objects, ref=mask[t:t+1, :num_objects+1]) + \
                                          mse_sim_loss(warpf, frame[0:1], (mask[0:1][:,1:].sum(1, keepdims=True) > 0))
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(testloader):
            ...
            for t in range(1, T):
                ...
                # memorize
                key, val, r4 = model(frames=frames[t-1:t], mask=tmp_mask, num_objects=num_objects)
                ...
                logits, ps, _, _, _ = model(frames=[frames[t-1:t], frames[t:t+1]], prev_mask=pred[-1], keys=tmp_key, values=tmp_val, 
                    num_objects=num_objects, max_obj=max_obj)
